...
Usage:
  python -I -S -B preflight-rv05.py [--spec launch-spec-rv05.json]
                                    ...
                                    [--execution-id <opaque id>]

Invocation contract (PAL_RV05_CONTROL_SAFETY_20260922, node N2, L6):
  - the argument surface is EXACTLY the three flags above: abbreviated
    flags (--spe, --exec...) are rejected (allow_abbrev=False) and any
    unknown argument is a hard usage error. This tool accepts no code, no
    policy-bypass switch and no alternative execution form from its
    caller; it is a read-only checker invoked by the launcher (or by a
    human) with fixed paths only.
  - --execution-id (opaque string, 8-64 chars of [0-9A-Za-z-]) is the
    per-invocation binding the LAUNCHER generates and later verifies:
    when provided it is echoed verbatim in the report's "execution"
    block, so a report left over from (or replayed by) an earlier run can
    never be mistaken for this run's fresh result.
...
Output: one JSON document on stdout:
  {schema, execution, generated_at_utc, overall: GO|NO_GO, checks: [{id,
   status, status_text, detail}], summary: {...}}
Every check status is one of PASS / FAIL / UNKNOWN. overall is GO only
when every check is PASS.

Execution provenance block (additive; consumed by the launcher's launch
boundary, PAL_RV05_CONTROL_SAFETY_20260922 L5/L6):

  "execution": {
    "execution_id": <the --execution-id value, or null when unprovided>,
    "spec": <absolute spec path this run actually read>,
    "spec_sha256": <sha256 of the spec FILE BYTES as read this run>,
    "json_out": <the --json-out path or null>,
    "argv": <this process's argv[1:], verbatim>,
    "invoked_at_utc": <run start time>
  }

The launcher compares execution_id / spec_sha256 / generated_at_utc
against its own invocation before it trusts any GO verdict, so a stale or
foreign GO report is rejected at the launch boundary (old-GO residue can
...
def main(argv=None) -> int:
    ...
    # allow_abbrev=False: the invocation contract is the EXACT three-flag
    # surface below — abbreviated flags (--spe) and unknown arguments are
    # hard usage errors (L6: no alternative/smuggled execution form).
    ...
    parser.add_argument(
        '--execution-id',
        help='opaque per-invocation id generated by the launcher; echoed '
             'verbatim in the report execution block so a stale/foreign '
             'report can never pass as this run\'s fresh result')
    args = parser.parse_args(argv)
    if args.execution_id is not None:
        if not re.fullmatch(r'[0-9A-Za-z-]{8,64}', args.execution_id):
            parser.error('--execution-id must be 8-64 chars of '
                         '[0-9A-Za-z-]')
    invoked_at_utc = _utcnow_iso()
    ...
    report = {
        ...
        # execution provenance (L5/L6 launch-boundary binding): consumed by
        # the launcher, which verifies execution_id / spec_sha256 /
        # generated_at_utc against its OWN invocation before trusting any
        # verdict. A report without a matching execution binding can never
        # authorize a launch.
        'execution': {
